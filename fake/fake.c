...
int main()
{
    ...
    char    *f2sName = "/tmp/f2s_mb";                       // filo names
    char    *s2fName = "/tmp/s2f_mb";                       //
	...
	FD_SET(STDIN, &rfds);
        ...
        if (select(s2f+1, &rfds, NULL, NULL, &tv) != 0) {   // wait until timeout
            if (FD_ISSET(s2f, &rfds)) {                     // something to read
                int nbchar;
                if (!(nbchar = read(s2f, serverRequest, MAXServerResquest))) break;
                serverRequest[nbchar]=0;
                fprintf(stderr,"%s", serverRequest);
                write(f2s, serverRequest, nbchar);
            }
	    if (FD_ISSET(STDIN, &rfds)) {                     // something to read
                int nbchar;
                if (!(nbchar = read(STDIN, serverRequest, MAXServerResquest))) break;
                serverRequest[nbchar]=0;
                write(f2s, serverRequest, nbchar);
            }
        }
